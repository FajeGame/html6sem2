...
class OrderService(
    ...
    fun create(userId: Long, dishIds: List<Long>): Order {
        return metrics.processingDuration.recordCallable {
            if (dishIds.isEmpty()) {
                metrics.recordBusinessError("validation")
                throw BadRequestException("dishIds must not be empty")
            }
            ...
            if (dishes.any { !it.isAvailable }) {
                metrics.recordOrderCreationFailed("stock_empty")
                throw OrderCreationException("One or more dishes are unavailable", "stock_empty")
            }
